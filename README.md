...
test-backend-dotnet-queue2 Dotnet service.
...
| ApplicationInsights__CloudRole          | Role used for filtering metrics    | no       |           |       | Set to `test-backend-dotnet-queue2-local` in docker compose files    |
...
`docker-compose -p test-backend-dotnet-queue2-test -f docker-compose.yaml -f docker-compose.test.yaml up`
...
`docker-compose -p test-backend-dotnet-queue2-test -f docker-compose.yaml -f docker-compose.test.watch.yaml up`
...
```
variableGroups:
    - test-backend-dotnet-queue2-snd1
    - test-backend-dotnet-queue2-snd2
    - test-backend-dotnet-queue2-snd3
variables:
    - test-backend-dotnet-queue2-APPLICATION-SECRET
```
